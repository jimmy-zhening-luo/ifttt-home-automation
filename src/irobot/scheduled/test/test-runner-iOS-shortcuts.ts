...
if (hoursSince(runs.last) < config.minHoursSinceLastRun) {
  IfNotifications.sendNotification.setMessage("robot would not have run: " + runs.last.toString());
  Irobot.cleanByRoom.skip();
} else {
  IfNotifications.sendNotification.setMessage("robot would have run: " + runs.last.toString());
  Irobot.cleanByRoom.skip();
}
